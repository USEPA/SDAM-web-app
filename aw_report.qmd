...
\textbf{Perennial indicator taxa: `r if (!is.null(params$aqua_inv)) {params$aqua_inv}`}
...
\textbf{Number of hydrophytic plant species: `r if (!is.null(params$hydro)) {params$hydro}`}
...
\textbf{Algal cover: `r ifelse(params$algal=='Not Detected', 'Not Detected', paste0(params$algal, '\\%'))`}

Check here if algae exclusively appears to have been deposited from an upstream source, and no local growth is evident: `r ifelse(params$algal_cb == 'TRUE', 'Checked', 'Not checked')`

Notes about algal cover:

`r params$notes_algal`
...
Vegetation differences photos and descriptions:
